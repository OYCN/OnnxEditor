...
void Parser::profile() {
  const int64_t opset_version =
      (mCtx->mOnnxModel.opset_import().size()
           ? mCtx->mOnnxModel.opset_import(0).version()
           : 0);
  ...
  auto ir_version = std::to_string(onnx_ir_major) + "." +
...
bool Parser::init() {
  ...
  if (mCtx->mTopoOrder.size() != nodes.size()) {
    LOG_ERROR("TopoSort size is " << mCtx->mTopoOrder.size()
                                  << ", But nodes size is " << nodes.size());
    return false;
  }
...
const Node_t& Parser::getNode(size_t idx) const {
  return mCtx->mNodes[mCtx->mTopoOrder[idx]];
}
...
std::vector<std::string>& Parser::getInitializers() const {
  return mCtx->mInitializers;
}
